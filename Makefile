...
release: clean build
	mkdir -p releases
	tar caf releases/$(name)-$(version)-$(target).tar.xz bin/batch extra/man/batch.1.gz extra/shell-completion/batch.bash extra/shell-completion/batch.zsh extra/shell-completion/batch.fish extra/shell-completion/batch-completions.nu
...
uninstall:
	rm -f $(DESTDIR)$(PREFIX)/bin/batch $(DESTDIR)$(PREFIX)/share/man/man1/batch.1.gz $(DESTDIR)$(PREFIX)/share/bash-completion/completions/batch.bash $(DESTDIR)$(PREFIX)/share/zsh/site-functions/_batch $(DESTDIR)$(PREFIX)/share/fish/vendor_completions.d/batch.fish $(DESTDIR)$(PREFIX)/share/nushell/vendor/autoload/batch-completions.nu
